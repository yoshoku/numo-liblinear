...
RSpec.describe Numo::Liblinear do
  describe 'constant values' do
    it 'has a version number', :aggregate_failures do
      expect(Numo::Liblinear::VERSION).not_to be_nil
      expect(Numo::Liblinear::LIBLINEAR_VERSION).not_to be_nil
    end

    it 'has solver type values', :aggregate_failures do
  ...
  describe 'classification' do
    ...
    let(:svc_model) { described_class.train(x, y, svc_param) }
    let(:logit_param) { { solver_type: Numo::Liblinear::SolverType::L2R_LR_DUAL, C: 1 } }
    let(:logit_model) { described_class.train(x, y, logit_param) }

    it 'predicts labels with C-SVC', :aggregate_failures do
      pr = described_class.predict(x_test, svc_param, svc_model)
    ...
    it 'performs 5-cross validation with C-SVC', :aggregate_failures do
      pr = described_class.cv(x, y, svc_param, 5)
  ...
  describe 'regression' do
    ...
    let(:n_samples) { x_test.shape[0] }
    let(:svr_param) do
      { solver_type: Numo::Liblinear::SolverType::L2R_L2LOSS_SVR_DUAL, C: 0.01, p: 3.0, random_seed: 1 }
    end
    ...
    it 'performs 5-cross validation with SVR', :aggregate_failures do
  ...
  describe 'errors' do
    ...
    describe '#train' do
      it 'raises ArgumentError when given non two-dimensional array as sample array' do
        expect do
          described_class.train(Numo::DFloat.new(3, 2, 2).rand, Numo::DFloat.new(3).rand,
                                svm_param)
      ...
      it 'raises ArgumentError when given non one-dimensional array as label array' do
        expect do
          described_class.train(Numo::DFloat.new(3, 2).rand, Numo::DFloat.new(3, 2).rand,
                                svm_param)
      ...
      it 'raises ArgumentError when the number of smaples of sample array and label array are different',
        ...
        expect do
          described_class.train(Numo::DFloat.new(5, 2).rand, Numo::DFloat.new(3).rand,
                                svm_param)
        end.to raise_error(ArgumentError, 'Expect to have the same number of samples for samples and labels.')
        expect do
          described_class.train(Numo::DFloat.new(3, 2).rand, Numo::DFloat.new(5).rand,
                                svm_param)
        end.to raise_error(ArgumentError, 'Expect to have the same number of samples for samples and labels.')
      end
      ...
      it 'raises ArgumentError when given invalid parameter value for libsvm' do
        svm_param[:C] = -100
        expect do
          described_class.train(Numo::DFloat.new(3, 2).rand, Numo::DFloat.new(3).rand,
                                svm_param)
        end.to raise_error(ArgumentError, 'Invalid LIBLINEAR parameter is given: C <= 0')
      end
    end
    ...
    describe '#cv' do
      it 'raises ArgumentError when given non two-dimensional array as sample array' do
        expect do
          described_class.cv(Numo::DFloat.new(3, 2, 2).rand, Numo::DFloat.new(3).rand, svm_param,
                             5)
        end.to raise_error(ArgumentError, 'Expect samples to be 2-D array.')
      end
      ...
      it 'raises ArgumentError when given non one-dimensional array as label array' do
        expect do
          described_class.cv(Numo::DFloat.new(3, 2).rand, Numo::DFloat.new(3, 2).rand, svm_param,
                             5)
        end.to raise_error(ArgumentError, 'Expect label or target values to be 1-D arrray.')
      end
      ...
      it 'raises ArgumentError when the number of smaples of sample array and label array are different',
         :aggregate_failures do
        expect do
          described_class.cv(Numo::DFloat.new(5, 2).rand, Numo::DFloat.new(3).rand, svm_param,
                             5)
        end.to raise_error(ArgumentError, 'Expect to have the same number of samples for samples and labels.')
        expect do
          described_class.cv(Numo::DFloat.new(3, 2).rand, Numo::DFloat.new(5).rand, svm_param,
                             5)
        end.to raise_error(ArgumentError, 'Expect to have the same number of samples for samples and labels.')
      end
      ...
      it 'raises ArgumentError when given invalid parameter value for libsvm' do
        svm_param[:C] = -100
        expect do
          described_class.cv(Numo::DFloat.new(3, 2).rand, Numo::DFloat.new(3).rand, svm_param,
                             5)
        end.to raise_error(ArgumentError, 'Invalid LIBLINEAR parameter is given: C <= 0')
      end
    end
    ...
    describe '#predict' do
      it 'raises ArgumentError when given non two-dimensional array as sample array' do
        expect do
          described_class.predict(Numo::DFloat.new(3, 2, 2).rand, svm_param,
                                  svm_model)
        end.to raise_error(ArgumentError, 'Expect samples to be 2-D array.')
      end
    end
    ...
    describe '#decision_function' do
      it 'raises ArgumentError when given non two-dimensional array as sample array' do
        expect do
          described_class.decision_function(Numo::DFloat.new(3, 2, 2).rand, svm_param,
                                            svm_model)
        end.to raise_error(ArgumentError, 'Expect samples to be 2-D array.')
      end
    end
    ...
    describe '#predict_proba' do
      it 'raises ArgumentError when given non two-dimensional array as sample array' do
        expect do
          described_class.predict_proba(Numo::DFloat.new(3, 2, 2).rand, svm_param,
                                        svm_model)
        end.to raise_error(ArgumentError, 'Expect samples to be 2-D array.')
      end
    end
    ...
    describe '#save_model' do
      it 'raises IOError when failed save file' do
        expect do
          described_class.save_model('', svm_param, svm_model)
